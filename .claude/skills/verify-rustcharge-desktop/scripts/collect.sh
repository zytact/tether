...
[ -f "$SCRATCH_HOME/config/dev.arnab.rustcharge.preview/settings.json" ] &&
  cp "$SCRATCH_HOME/config/dev.arnab.rustcharge.preview/settings.json" "$out/settings.json"
...
awk -v server="destination=$server " '
  ...
  END { emit() }
' "$out/notifications.raw" | grep '^app=Rustcharge Preview' >"$out/notifications.txt" || true
...
awk '
  /^@ / { at = $2 }
  /Sink Input #/ { id = $3 }
  /application.process.binary = "rustcharge-preview"/ { if (!(id in first)) first[id] = at; last[id] = at }
...

{
  ...
  echo "rustcharge-preview audio streams: $(wc -l <"$out/audio-streams.txt")"
