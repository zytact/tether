...
XVFB="${RUSTCHARGE_XVFB:-$(command -v Xvfb || true)}"
[ -x "$BIN" ] || { echo "Preview binary missing. Run build-preview.sh first." >&2; exit 1; }
[ -x "$XVFB" ] || { echo "Xvfb is required. Install it or set RUSTCHARGE_XVFB." >&2; exit 1; }
...
if [ "$RESTART" = 1 ]; then
  ...
  if [ -n "$SEED" ]; then
    mkdir -p "$SCRATCH_HOME/config/dev.arnab.rustcharge.preview"
    printf '%s\n' "$SEED" >"$SCRATCH_HOME/config/dev.arnab.rustcharge.preview/settings.json"
  fi
fi
...
preview_env=(env -u ELECTRON_RUN_AS_NODE -u RUSTCHARGE_POWER_SUPPLY DISPLAY="$display"
  XDG_CONFIG_HOME="$SCRATCH_HOME/config")
[ "$MODE" = fake ] && preview_env+=(RUSTCHARGE_POWER_SUPPLY="$POWER_SUPPLY")
...
for _ in $(seq 1 30); do
  ...
  if curl -fsS "http://127.0.0.1:$cdp_port/json/list" 2>/dev/null | grep -q '"type": "page"'; then
    echo "Ready: Rustcharge Preview ($MODE battery) on display $display (pid $preview_pid, DevTools port $cdp_port)"
    exit 0
  fi
  sleep 1
done
