---
name: verify-rustcharge-desktop
description: Build and drive the isolated Rustcharge Preview Electron tray app (desktop/) against a fake battery, then capture proof of its notifications, urgency, sound, settings window, persistence, and tray menu. Use when verifying a change under desktop/, such as alert thresholds, alert sessions, settings changes, sound playback, open at login, or the tray.
---

# Verify Rustcharge Desktop

Rustcharge Desktop is an Electron tray app in `desktop/`. The main process reads the battery on an interval, decides alerts in `src/main/alerts.ts`, sends notifications, and plays the chosen sound in a hidden window. The window is a settings page. Every change there saves to `settings.json` and applies to the running monitor at once. There is no CLI.

Verification uses the built preview app on Linux. Drive its real window over the Chrome DevTools Protocol, its tray menu over D-Bus, and its battery through a fake sysfs tree. Do not load `dist/index.html` or the dev server in a browser, and do not stub `window.rustcharge`: a plain tab has no preload bridge and no monitor.

For the Rust CLI in the repository root, use `verify-rustcharge` instead.

## Isolation

`desktop/src/main/identity.ts` gives the preview its own product name `Rustcharge Preview`, app id `dev.arnab.rustcharge.preview`, and executable `rustcharge-preview`, so its settings, autostart entry, and single-instance lock never touch an installed release.

`launch.sh` adds three more layers:

- An Xvfb display, so the window never appears on the desktop.
- `XDG_CONFIG_HOME` under `/tmp/rustcharge-desktop-verify/home/config`, so settings and the autostart entry land in scratch space.
- `RUSTCHARGE_POWER_SUPPLY` pointing at a fake sysfs tree under `/tmp/rustcharge-desktop-verify/power_supply`. Only preview builds read it. The tree also holds a 5% wireless-mouse battery with a `Device` scope, which the app must skip.
...
Only one harness preview runs at a time. `launch.sh` refuses a second launch until `cleanup.sh` runs. Never kill Rustcharge processes by name: the user may run the release app or the Rust CLI daemon.
...
```sh
S=.agents/skills/verify-rustcharge-desktop/scripts
EVIDENCE=verification-evidence/desktop-$(date -u +%Y%m%dT%H%M%SZ)-<name>
mkdir -p "$EVIDENCE"
...
Ready means `launch.sh` prints `Ready: Rustcharge Preview` and `doctor.sh` ends with `DOCTOR: worth driving`. The fake battery starts at `Discharging 50%`, which crosses no default threshold.
...
`build-preview.sh` runs `vp build`, `vp pack`, and `node scripts/package.ts --preview --dir` in `desktop/`, installing Electron's binary first when a fresh install skipped it. Rebuild after every source change; doctor fails when `desktop/src` is newer than the binary.
...
```sh
$S/tray.sh layout
$S/tray.sh click "Open Rustcharge Preview"
...
| `audio-streams.txt` | `rustcharge-preview` sink inputs, with first and last time seen |
...
It stops only the preview, Xvfb, and the two recorders that `launch.sh` recorded by PID and executable, then removes `/tmp/rustcharge-desktop-verify`. Evidence under `verification-evidence/` (gitignored) survives. Run cleanup after every failed attempt too.
