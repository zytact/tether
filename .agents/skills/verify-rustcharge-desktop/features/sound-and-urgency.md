...
Proof: `notifications.txt` shows `urgency=2`, `audio-streams.txt` lists a `rustcharge-preview` stream inside the alert window, and `preview.log` has no `Failed to play`. After `Clear`, the row reads `The system notification sound.` and `settings.json` holds `"soundPath": null`.
