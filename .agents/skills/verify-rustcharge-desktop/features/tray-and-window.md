...
The monitor lives in the tray. Its menu leads with a disabled line for the latest reading, such as `12% · Not charging`, `Battery unavailable`, or `Checking the battery` before the first read, then `Open Rustcharge Preview` and `Quit`. Closing the window destroys it and keeps the monitor running. Opening it again from the tray builds a fresh window. A second launch raises the running instance's window instead of starting another.
...
$S/tray.sh click "Open Rustcharge Preview" && node $S/drive.ts snapshot | head -3
